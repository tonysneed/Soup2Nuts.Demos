...
namespace SoupToNuts.Final.ConsoleClient
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // Create http client
            const string serviceBaseAddress = "http://localhost:" + "58141" + "/";
            var client = new HttpClient { BaseAddress = new Uri(serviceBaseAddress) };

            // Get products
            Console.WriteLine("Press Enter for Products:");
            Console.ReadLine();
            IEnumerable<Product> products = GetProducts(client);
            if (products == null) return;
            foreach (var p in products)
                PrintProduct(p);

            // Get customers
            Console.WriteLine("\nPress Enter for Customers:");
            Console.ReadLine();
        ...
        private static IEnumerable<Product> GetProducts(HttpClient client)
        {
